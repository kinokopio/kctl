...
示例：
  sa                    列出所有 SA
  sa --admin            只显示 cluster-admin
  sa --risky            只显示有风险的 SA
  sa -n kube-system     只显示 kube-system 命名空间的 SA`
}
...
func (c *SACmd) Execute(sess *session.Session, args []string) error {
	...
	// 检查是否已扫描
	if !sess.IsScanned {
		return fmt.Errorf("请先执行 'scan' 扫描 ServiceAccount")
	}
	...
	if len(sas) == 0 {
		p.Warning("没有找到 ServiceAccount，请先执行 'scan'")
		return nil
	}
...
func (c *SACmd) buildFlags(p output.Printer, securityFlagsJSON string, isClusterAdmin bool) string {
	var flags []string
...
func (c *SACmd) formatPermissions(p output.Printer, permissionsJSON string) string {
	// 简单格式化权限
